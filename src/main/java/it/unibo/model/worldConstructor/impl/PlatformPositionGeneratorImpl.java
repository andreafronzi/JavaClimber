 ...
 * Generates random positions for new platforms within a specified range (defined by difficulty)
...
public class PlatformPositionGeneratorImpl implements PlatformPositionGenerator {
    ...
    private void genPosX(final Vector2d pos) {
        while (pos.getX() > previousPlatformPosition.getX() + difficult.maxDistanceX() && pos.getX() < previousPlatformPosition.getX() - difficult.maxDistanceX()) {
    ...
    private void genPosY(final Vector2d pos) {
        while (pos.getY() > previousPlatformPosition.getY() + difficult.maxDistanceY()
            ...
            pos.setY(randomNumber.nextDouble(previousPlatformPosition.getY() + difficult.minDistanceY(), previousPlatformPosition.getY() + difficult.maxDistanceY()));
        }
    }
